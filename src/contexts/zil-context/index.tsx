...
export class ZilProvider extends React.Component {
  ...
  initState = (networkKey?: string) => {
    let curNetworkKey = networkKey || NETWORK.MainNet;

    const urlSearchParams = new URLSearchParams(window.location.search);
    const params = Object.fromEntries(urlSearchParams.entries());
    const networkParam = params['network'];
    const isValidNetwork = [NETWORK.TestNet, NETWORK.MainNet].includes(
    ...
    const curNetwork =
      curNetworkKey === NETWORK.MainNet ? config[NETWORK.MainNet] : config[NETWORK.TestNet];
